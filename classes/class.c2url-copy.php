...
$cpdate = date("Y-m-d");
if(!file_exists($proxyfile)){// if a proxy doesnt eexist return error msg
			...
			if($no>$noofserver){ // if excel row count is greater than total no of servers available
					...
					//=====================================================================================================
						if(!file_exists($proxyfile)){
							...
							while(!$nocapcha){ 
								$tempdata = self::getproxyfilearray($proxyfile);// open file and get array
								...
								if($querylmtpy>$currentstatuspy){// To check the no of query does not exceed the limit
											...
											if(self::checkcap($tempreturndata)){ // check if capatcha
												self::updatedeactive($proxyfile); //captacha arrived so deactive
						...
						if(!file_exists($proxyfile)){
							...
							while(!$nocapcha){ // Do Till Capatcha or End of query
								...
								if($querylmtpy>$currentstatuspy){ // If query limit is greater that daily queried limit
											...
											if(self::checkcap($tempreturndata)){ // check if capatcha
												self::updatedeactive($proxyfile);  // if capatcha update deactive
												$ctr--;
												$nocapcha = false;
													if($ctr==0){
														$nocapcha = true;
	...
	public static function checkcapserver($serverno){
			$fhRD = fopen("../proxy/captcha.inf", 'r') or die("Failed to read file"); 
			$fhr = fgets($fhRD);
			fclose(fhRD);
			$fhrexp = explode('|', $fhr);
...
public static function curl_proxy($sURL,$sProxyUrl,$no){
	$proxyfile_auth = "../proxy/password-".$no.".inf";
	
	if(file_exists($proxyfile_auth)){
		$proxy_file = self::getproxyfilearray($proxyfile_auth);
...
public static function getproxyfilearray($proxyfile){
