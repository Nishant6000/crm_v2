...
class verifyEmail {
	...
	public function check($email) {
		...
		foreach ($mxs as $host) {
			...
			if ($this->stream === FALSE) {
				...
				if ($this->_streamCode($this->_streamResponse(1)) == '220') {
		...
		$this->_streamResponse(2);
		$this->_streamQuery("MAIL FROM: <{$this->from}>");
		$this->_streamResponse(3);
		$this->_streamQuery("RCPT TO: <{$email}>");
		//$code = $this->_streamCode($this->_streamResponse(4));
		$this->_streamResponse(4);
		$this->_streamQuery("RSET");
		$this->_streamResponse(5);
	 ...
	 */
	protected function _streamResponse($statno, $timed = 0) {
		$errret = "0";
		...
		$this->edebug($reply);
		if(strstr($reply, 'Protocol error') && $statno==2){
			echo "ERROR 24";
			$errret = "1";
		}else if(strstr($reply, 'The email account that you tried to reach does not exist') && $statno==4){
			echo "ERROR 25";
			$errret = "1";
		}else{
			if ($errret!=="1"){
