...
if(!file_exists($proxyfile)){
						...
						if(!file_exists($proxyfile)){
							...
							while(!$nocapcha){
								...
								if($querylmtpy>$currentstatuspy){
											...
											if(checkcap($tempreturndata)){ // check if capatcha
												...
												$ctr--;
												$currentstatuspy++;
												updatecstatus($proxyfile,$currentstatuspy);
											}
								}else{
	...
	function queryproxy($serverport, $status, $search2){
		if ($status == 1){
		$url = "https://www.google.com/search?hl=en&q=". $search2 ."&btnG=Google+Search";
