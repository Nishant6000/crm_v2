...
class validateEmail {
	 ...
	 * @var string 
	 */
	protected $from = 'info@researchtool.online';
	...
	public function check($email) {
        ...
        public function smtpval($email){
		$this->_streamQuery("HELO " . self::parse_email($email));
        $resp = $this->_streamResponse();
        if (strstr($resp,"Protocol error")){
            return 21;
        }
		$this->_streamQuery("MAIL FROM: <{$this->from}>");
        $this->_streamResponse();
		$this->_streamQuery("RCPT TO: <{$email}>");
		$code2 = $this->_streamResponse();
		$code = $this->_streamCode($code2);
		//$this->_streamResponse()
		$bfstat1 = "Access denied, banned sending";
		$bfstat2 = "The email account that you tried to reach does not exist";
		if(preg_match("/{$bfstat1}/i", $code2)){
			return 23;
		}else if(preg_match("/{$bfstat2}/i", $code2)) {
			return 24;
		}else if($code != "250"){
           return 22;
        }
	...
	public static function mailtester($email){
		$stat1 = "Unknown response from mail server";
		$stat2 = "E-mail address does not exist on this server";
		$stat3 = "The domain is invalid or no mail server was found for it";
		$stat4 = "E-mail address is valid";
		$stat5 = "Server doesn't allow e-mail address verification";
		$post = [
			'lang' => 'en',
			'email' => $email,
		];
		
		$ch = curl_init("https://mailtester.com/index.php");
		...
		foreach ($arr as $arr2){
			$testarray2 = $arr2->nodeValue;
		}
		if (preg_match("/{$stat1}/i", $testarray2)){
			return "Good";
		}else if (preg_match("/{$stat2}/i", $testarray2)){
			return "Bad";
		}else if (preg_match("/{$stat3}/i", $testarray2)){
			return "Bad2";
		}else if (preg_match("/{$stat4}/i", $testarray2)){
			return "Excellent";
		}else if (preg_match("/{$stat5}/i", $testarray2)){
			return "Unknown";
		}else{
			return $testarray2;
		}
